...
class Graine extends MY_Controller
{
	...
	public function upload_proof()
	{
		...
		if ($beneficiaire['id_membre_beneficiaire']==0) {
			...
			$msg_mail="Cher Administrateur du système,<br> Nous tenons à vous informer que ".$donateur['nom_membre']." ".$donateur['prenom_membre']." vient de charger le preuve de paiement du cadeau destiné au compte solidarité Umoja.<br>Cordialement";
		...
		$msg_mail="Cher(e) ".$receveur['nom_membre']." ".$receveur['prenom_membre'].",<br> Nous tenons à vous informer que ".$donateur['nom_membre']." ".$donateur['prenom_membre']." vient de charger le preuve de paiement de votre cadeau.Vous pouvez vous connecter <a href=".base_url().">ici</a> pour accuser la réception de ce cadeau.<br>Cordialement"; 
    ...
    $description="Chargemenent d'un preuve pour le gift donner à ".$activite_detail;
	...
	public function confirm_proof()
	{
		...
		$msg_mail="Cher(e) ".$donateur['nom_membre']." ".$donateur['prenom_membre'].",<br> Nous tenons à vous informer que ".$receveur['nom_membre']." ".$receveur['prenom_membre']." vient d'approuver  la réception de votre cadeau.Vous pouvez vous connecter <a href=".base_url().">ici</a> pour plus de détail.<br>Cordialement"; 
		...
		$action='Preuve de paiement';
    $description="Validation d'un preuve pour le gift payé par ".$activite_detail;
    ...
    $this->Save_history($action,$description);
		...
		$data['sms']='<div style="background-color:#42ba96" class="alert alert-success text-center text-success" id ="sms">Vous venez de charger la réception du cadeau.</div>';
		$this->session->set_flashdata($data);
